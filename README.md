# 👋 Hi, I'm Anton

Software Engineer building reliable and maintainable products.

## 🚀 About me
- Full-cycle product development experience
- Focus on clean architecture and long-term maintainability
- Comfortable working in distributed teams

## 🛠 Tech Stack

**Frontend:** TypeScript, React, React Native, Expo, Vite

**State Management:** Zustand, Redux Toolkit, TanStack Query

**Realtime & APIs:** WebSockets, REST APIs, Axios

**Mobile:** Push Notifications, Deep Linking, EAS Build

**Tools:** Git, Docker, Linux, Postman, CI/CD

**Currently Exploring:** Django, AI Integrations, Fullstack Development, Product Design

## 📂 Projects
Mobile app for hospitality business 
[Google Play](https://play.google.com/store/apps/details?id=com.sustainstay.sustainstay&hl=en)
[App Store](https://apps.apple.com/us/app/sustainstay/id6737006360)

## 📊 Languages
[![Anurag's GitHub stats](https://github-stats-extended.vercel.app/api?username=octocat&theme=AntBezr)]

---

- 📫 How to reach me: [LinkedIn](https://www.linkedin.com/in/antonbezruchenkov/)

